...
import { Lightbulb, ChevronsRight, Timer } from 'lucide-react';
import { Button } from "../ui/button";

export function ScheduleItem({ task }: { task: ScheduleTask }) {
  ...
  return (
    <div className={cn("bg-card border rounded-lg transition-all relative overflow-hidden p-4", task.completed && "bg-accent/50 border-primary/30")}>
      <div className="flex items-start space-x-4">
        ...
        <div className="flex-1 space-y-2">
          ...
          {!task.completed && (
              <Button variant="outline" size="sm" onClick={() => setFocusedTask(task)}>
